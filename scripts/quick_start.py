...
def main():
    ...
    if not (data_dir / 'br35h').exists():
        print("ERROR: br35h dataset not found!")
        print(f"Please ensure the br35h folder exists in {data_dir}")
        return
    
    if not (data_dir / 'sartaj').exists():
        print("ERROR: sartaj dataset not found!")
        print(f"Please ensure the sartaj folder exists in {data_dir}")
        return
    ...
    # Run training
    train_script = project_root / 'scripts' / 'train_gnn.py'
    cmd = [
        sys.executable,
        str(train_script),
        '--dataset', dataset,
